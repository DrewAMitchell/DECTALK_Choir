...
- ``[MM:SS]`` or ``[MM:SS|duration]`` at the start of each drafted line.
...
def sanitize_transcript_line(line):
	"""Keep lyric syntax while removing punctuation that breaks word lookup."""
	stripped = line.strip()
...
def read_lyric_token_lines(path, sanitize=False):
	if not path.exists():
	...
	for line in path.read_text().splitlines():
		if sanitize:
			line = sanitize_transcript_line(line)
		stripped = line.strip()
		if len(stripped) == 0 or stripped.startswith('#'):
			continue
		tokens = []
...
def read_transcript_lines(path):
	...
	for line_number, raw_line in enumerate(path.read_text().splitlines(), start=1):
		line = sanitize_transcript_line(raw_line)
		stripped = line.strip()
		if len(stripped) == 0 or stripped.startswith('#'):
			continue
		...
		duration_ms = None
		if parts and parts[0].startswith('[') and parts[0].endswith(']'):
			timing_token = parts.pop(0)
			try:
				timing = pp.parseLineTimingToken(timing_token)
			except ValueError as error:
				raise SystemExit(f"Invalid timing token on line {line_number} in {path}: {error}") from error
			start_ms = timing[1]
		...
		for raw_token in parts:
			...
			if raw_token.startswith('!'):
				try:
					...
					raise SystemExit(f"Invalid repeat token '{raw_token}' in {path}") from error
				continue
		...
		if tokens:
			for repetition in range(lyric_repetitions):
				lines.append(
					TranscriptLine(
						tuple(tokens),
						timing_token if repetition == 0 else None,
						start_ms if repetition == 0 else None,
						duration_ms if repetition == 0 else None,
					)
				)
		lyric_repetitions = 1
...
def format_timestamp_ms(timestamp_ms):
	"""Format generated line starts in the transcript-friendly [MM:SS] form."""
...
def line_start_timestamps(source_lines, notes, tight_gap_ms, word_gap_ms):
	"""Estimate line starts from the note allocation used by line-aware drafting."""
	line_note_counts = allocate_line_note_counts(source_lines, notes, tight_gap_ms, word_gap_ms)
	timestamps = []
	note_index = 0
	for note_count in line_note_counts:
		if note_index < len(notes):
			timestamps.append(notes[note_index].start_ms)
		elif notes:
			timestamps.append(notes[-1].end_ms)
		else:
			timestamps.append(0)
...
def render_draft(
	...
	if auto_lines:
		line_starts = [phrase[0].start_ms for phrase in phrases if phrase]
		line_metadata = [None for _ in token_lines]
	else:
		line_starts = line_start_timestamps(source_lines, notes, tight_gap_ms, word_gap_ms)
		line_metadata = list(transcript_lines or [])
	...
	for line_index, tokens in enumerate(token_lines):
		...
		if include_comments:
			start_ms = round(line_starts[line_index]) if line_index < len(line_starts) else 0
			output_lines.append(
				f"# line {line_index + 1}: start={start_ms}ms notes="
				f"{sum(token.note_count for token in tokens)} words={len(tokens)}"
			)

		if (
			line_index < len(line_metadata)
			and line_metadata[line_index] is not None
			and line_metadata[line_index].timing_token
		):
			prefix = line_metadata[line_index].timing_token
		else:
			start_ms = line_starts[line_index] if line_index < len(line_starts) else 0
...
def render_placeholder_draft(notes, phrases, placeholder, include_comments=True):
	...
	for phrase_index, phrase_notes in enumerate(phrases):
		...
		if include_comments:
			output_lines.append(
				...
				f'notes={len(phrase_notes)} placeholders={len(phrase_notes)}'
			)
		output_lines.append(' '.join(placeholder for _ in phrase_notes))
	return output_lines
...
def build_arg_parser():
	...
	comment_group.add_argument(
		...
		action='store_true',
		help="Include diagnostic line comments in the drafted lyrics.",
	)
	comment_group.add_argument(
		'--no-comments',
		dest='include_comments',
		action='store_false',
		help="Do not include diagnostic comments in the drafted lyrics (default).",
	)
...
def main():
	...
	if args.placeholder is not None:
		...
		transcript_lines = read_transcript_lines(source_path)
		source_lines = [list(line.words) for line in transcript_lines]
		source_line_count = len(source_lines)
		effective_auto_lines = args.auto_lines
		if (
			not effective_auto_lines
			and len(source_lines) == 1
			and transcript_lines
			and not any(line.timing_token for line in transcript_lines)
		):
			# A single untimestamped block is a bulk transcript, not one musical
			# phrase. Let MIDI rests provide the human-editable line boundaries.
			effective_auto_lines = True
	...
	print(f"Lyric source: {source_path or 'generated note-level placeholder'}")
	...
	print(f"Phrases: {len(phrases)}   Source lines: {source_line_count}")
