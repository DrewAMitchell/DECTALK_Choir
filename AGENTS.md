...
- The VS Code `DECTALK Choir Studio (Tauri + CDP)` launch keeps WebView2 diagnostics on loopback at `http://127.0.0.1:9356`. Use its `/json` targets for DOM, console, layout, and screenshot automation instead of driving the visible DevTools window. This flag is development-only and must not be added to release startup.
- During live UI verification, prefer CSS-only fixes when behavior does not require React changes. Vite can hot-reload styles without remounting Studio or discarding the verifier's in-memory workflow state.
...
Supported YAML keys live in `pyFuncs/SettingsSchema.py`. `choir.py` prints full-path warnings for unsupported top-level, track, spectrogram, and spectrogram-video keys before rendering; update the schema and its tests whenever a runtime gains a real setting. Do not retain dead tuning keys merely for compatibility because YAML otherwise accepts them silently.
...
- `EXPORT_PHONEME_STRING`: per-track portable command export. Align owns the checkbox; the next audio render writes one timing-complete command to `outputs/_phonemes/<Part>.txt`. The exporter inserts rests between phrases and fails rather than flattening overlapping phrases or normal-speech (`~word`) segments.
- `SPECTROGRAM`: per-track visual ownership boundary. Its children contain `COLOR_HSB`, fractional `[size, left, top]` `POSITION`, optional label/voice/head-size fields, independent label/current-word font and size controls, and current-word color/display settings. Voice labels use the readable built-in speaker name plus directive for every supported voice, not only Perfect Paul. Do not add new flat `VID_*` settings; legacy flat values are read only for migration.
...
- `tools/split_polyphonic_midi.py` safely expands every overlapping-note source track into the minimum number of monophonic voice tracks. It preserves note spans/controllers, gives every note-bearing output track a stable name and unique non-percussion MIDI channel for reliable DAW import, verifies note preservation and channel separation, and never overwrites its source MIDI. CLI callers can opt back into source channel routing with `--preserve-channels`.
- Studio exposes polyphonic splitting from affected items in both the Align track rail and the right side of the Align toolbar. Inspection polyphony and note counts are computed dataclass properties, so the bridge serializer must project them explicitly. The modal analyzes the selected configured track without writing files, previews the resulting lane names, ranges, and note counts, and invokes the established splitter only after explicit export. Replacing the working MIDI is explicit, creates a unique restorable `.bak` for that operation, preserves the original role on voice one, adds later voices as disabled/undrafted roles, reloads the song rail, and invalidates stale alignment maps. The completion view can restore that backup and preserves the displaced split under `outputs/recovery/`. A separate export receives the same DAW-compatible MIDI structure under `outputs/midi_exports/` but does not activate it or alter Studio roles; never place inactive `.mid` exports in `inputs/`, where deterministic MIDI discovery could select them. Do not make Align mutate MIDI implicitly. The renderer sequentializes overlaps and can tolerate short transitions, but simultaneous notes collapse to zero duration; it does not produce true within-role chord polyphony.
- Align lists note-bearing tracks from the working MIDI that do not yet have a configured role. **Add MIDI track** creates a disabled role plus a comment-only lyric placeholder without modifying the MIDI. Removing a rail item deletes only that role from `settings.yaml`; it preserves the source MIDI and all lyric, candidate, alignment, and output artifacts, so the role can be added again later.
...
- Studio validates MIDI preview paths inside its workspace, then stages playback through a short process-specific file under the system temp directory. Windows MCI's legacy sequencer can report a valid MIDI as missing when its original absolute path approaches 128 characters; do not remove this staging boundary. Align persists one General MIDI preview instrument in UI state. Preview generation strips source program changes, applies that program to every note channel, and remaps percussion channel 10 only in the temporary preview so role-to-role playback remains consistent without mutating source MIDI.
- Normal renders load MIDI with raw parser diagnostics disabled; controller, pitch-wheel, marker, and karaoke metadata do not belong in the Studio render log. Native Python subprocesses must emit UTF-8, and the native stream reader must retain its lossy-decoding fallback because imported MIDI metadata can contain legacy Windows code-page bytes such as a copyright symbol.
...
- Per-track clips under `outputs/_animation/` are removed only after ffmpeg successfully creates a non-empty final `<Song>.mp4`; retain them when composition fails. Current-word overlays read word cues from the applied alignment sidecar, with the active draft report as fallback. Treat `CURRENT_WORD_ENABLED` as a render contract: fail preflight before worker rendering if either source provides no timing cues.
