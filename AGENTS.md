...
- Studio exposes polyphonic splitting from affected items in the Align track rail. Its modal analyzes the selected configured track without writing files, previews the resulting lane names, ranges, and note counts, and invokes the established splitter only after explicit export. Other MIDI tracks pass through unchanged. Replacing the working MIDI is explicit, creates a one-time `.bak` backup, preserves the original track name on voice one, and invalidates stale alignment maps so they rebuild against the new notes. Do not make Align mutate MIDI implicitly.
