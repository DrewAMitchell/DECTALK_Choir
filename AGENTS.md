...
- Align permits permanent Ctrl-click deletion of an individual MIDI source track from its rail. The bridge verifies a temporary rewrite before atomically replacing the working MIDI, but intentionally retains no backup; users own their source copy. Preserve the configured role, lyrics, tuning, and candidates so the missing source remains visible. Refuse inline deletion when roles share the source track or when that track owns tempo/time-signature events.
