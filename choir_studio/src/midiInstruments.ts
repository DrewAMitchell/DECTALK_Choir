export const midiInstrumentGroups = [
...
export function midiInstrumentName(program: number): string {
  const group = midiInstrumentGroups[Math.floor(program / 8)];
  return group?.[1][program % 8] ?? "Acoustic Grand Piano";
}
