...
export default function App() {
  ...
  return <main className="studio-shell">
    ...
    <section className={`workspace ${stage === "review" ? "review-workspace" : ""}`}>
      <aside className="track-rail"><h2 className="rail-song-title" title={song}>{song || "Song"}</h2><div className="rail-heading"><PanelLeft size={16} /> Tracks</div><div className="track-list">{inspection?.roles.map((item) => <div key={item.role} className={`track-entry${item.role === roleName ? " active" : ""}`}><button className="track" onClick={() => selectRole(item.role)} aria-pressed={item.role === roleName}><span className="track-copy"><strong title={`${item.role} · ${item.midi_source_name}`}>{item.role}</strong><span className="track-overlap-slot" aria-hidden="true" /><RailPitchRange value={item.midi_range} /></span><span className="track-note-total" title={`${item.note_count} MIDI notes`}><b>{item.note_count}</b><small>notes</small></span>{item.role === roleName && <span className="track-selected-marker" title="Selected track" aria-label="Selected track"><CircleCheck size={15} /></span>}</button>{item.polyphony && item.polyphony > 1 ? <button type="button" className="track-overlap-badge warning" onClick={() => { if (item.role !== roleName) selectRole(item.role); setSplitRoleName(item.role); }} title={`${item.polyphony}-note overlap detected. Open the monophonic track splitter.`} aria-label={`Split overlapping MIDI track for ${item.role}`}><CircleAlert size={10} />{item.polyphony}-note overlap</button> : <span className="track-overlap-badge" title={item.polyphony === 1 ? "No simultaneous notes detected" : "Note overlap unavailable"}><GitBranch size={10} />{item.polyphony === 1 ? "No note overlap" : "Overlap --"}</span>}</div>)}</div></aside>
...
function AlignStage({ role, inspection, song, alignment, loading, templateSources, onAdoptTemplate, onOpenLyrics, onOpenSplitter, onApplied, setAlignment, selectedPhrase, setSelectedPhrase, busy, setBusy, setError }: { role: Role | null; inspection: SongInspection | null; song: string; alignment: AlignmentState | null; loading: boolean; templateSources: AlignmentTemplate[]; onAdoptTemplate(sourceRole: string): void; onOpenLyrics(): void; onOpenSplitter(): void; onApplied(): Promise<void>; setAlignment(value: AlignmentState | null): void; selectedPhrase: number | null; setSelectedPhrase(value: number): void; busy: string; setBusy(value: string): void; setError(value: string): void }) {
  ...
  const [applied, setApplied] = useState<{ transcriptCreated: boolean } | null>(null);
  ...
  const apply = async () => {
    ...
    try {
      const result = await bridge<{ transcript_created: boolean; source_in_sync: boolean }>({ command: "apply_alignment", song, role: role.role, text: alignment.text });
      setAlignment({ ...alignment, source_in_sync: result.source_in_sync }); setApplied({ transcriptCreated: result.transcript_created }); setApplyArmed(false); await onApplied();
    } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); } finally { setBusy(""); }
  };
  ...
  return (
    <section className="align-workspace">
      {applyArmed && alignment && <section className="apply-confirm"><div><strong>Publish aligned lyrics?</strong><span>`choir.py` renders only the configured lyric file. The original transcript is preserved separately when needed.</span></div><button className="secondary" onClick={() => setApplyArmed(false)} disabled={!!busy}>Cancel</button><button className="primary" onClick={() => void apply()} disabled={!!busy}>Apply to source</button></section>}
      {applied && sourceInSync && <div className="notice alignment-applied"><CircleCheck size={17} /><div><strong>Track lyrics are synchronized.</strong>{applied.transcriptCreated && <> The original transcript was preserved beside the aligned lyric file.</>}</div></div>}
