...
MacinTalk support is being explored as a separate backend before it is wired into choir.py. The standalone Windows probe uses the WinTalker NVDA add-on's render DLL directly, without launching NVDA.

```powershell
powershell -ExecutionPolicy Bypass -File .\tools\setup_wintalker_backend.ps1
.\.venv\Scripts\python.exe tools\macintalk_standalone.py --list-voices
.\.venv\Scripts\python.exe tools\macintalk_standalone.py --voice fred --text "Hello from MacinTalk." --output outputs\macintalk_probe\fred.wav
```

See **docs/macintalk-vs-dectalk.md** for the current backend comparison and integration plan.
...
**PITCH_VOLUME_BOOST_START / PITCH_VOLUME_BOOST_DB_PER_SEMITONE / PITCH_VOLUME_BOOST_MAX_DB**: Optional pitch-dependent gain curve for tracks that get quiet on high DECtalk pitches. Pitches above the start value are boosted by the per-semitone amount, capped by the max dB value.

**SEGMENT_NORMALIZE_PITCH_START / SEGMENT_NORMALIZE_TARGET_DBFS / SEGMENT_NORMALIZE_MAX_BOOST_DB**: Optional measured loudness lift for quiet generated phoneme segments. Pitches at or above the start value are raised toward the target dBFS, capped by the max boost and an internal peak guard. Set max boost to 0 to disable.
